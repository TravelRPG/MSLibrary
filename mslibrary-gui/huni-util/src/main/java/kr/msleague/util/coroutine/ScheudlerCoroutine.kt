...
import kr.msleague.util.coroutine.SynchronizationContext.ASYNC
import kr.msleague.util.coroutine.SynchronizationContext.SYNC
...
class BukkitSchedulerController(val plugin: Plugin, val scheduler: BukkitScheduler) : Continuation<Unit> {
    ...
    suspend fun switchContext(context: SynchronizationContext): Boolean =
        suspendCoroutine { cont -> schedulerDelegate.doContextSwitch(context, cont::resume) }

    suspend fun newContext(context: SynchronizationContext): Unit =
        suspendCoroutine { cont -> schedulerDelegate.forceNewContext(context, { cont.resume(Unit) }) }
